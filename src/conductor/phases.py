...
from conductor.gh_sync import add_label, post_comment
...
class PhaseResult:
    def __init__(
        self, phase: str, success: bool, error: str | None = None
    ) -> None:
        self.phase = phase
...
def run_phase(ctx: PhaseContext, phase: str) -> PhaseResult:
    ...
    if handler is None:
        return PhaseResult(
            phase=phase, success=False, error=f"Unknown phase: {phase}"
        )
    return handler(ctx)


def run_all_phases(
    ctx: PhaseContext, start_phase: str = "design"
) -> list[PhaseResult]:
...
def _dispatch_swarm(
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        futures = {
            executor.submit(_run_one, sub_id, data): sub_id
            for sub_id, data in inputs
        }
...
def _load_issue_context(ctx: PhaseContext, phase: str) -> IssueContext:
    """Load issue context from local DB (cached from GitHub during sync)."""
    row = ctx.db.get_issue(ctx.issue_number)
    if row is None:
        msg = f"Issue #{ctx.issue_number} not found in local DB"
...
def _resolve_timeout(ctx: PhaseContext, step_id: str) -> float:
    """Resolve timeout for a step from config, falling back to 300s default."""
    phase_prefix = step_id.split(".")[0]
    phase_names = dict(enumerate(PHASE_ORDER, 1))
    phase_name = phase_names.get(int(phase_prefix), "")
    timeout = ctx.config.timeouts.get(phase_name, 300)
...
def _run_architect(ctx: PhaseContext) -> PhaseResult:
    ...
    try:
        ...
        matrix_result = _phase_dispatch(
            ctx, "3.2", issue_ctx, TestMatrix
        )
        if not matrix_result.success:
            return PhaseResult(
                "architect", False, error=matrix_result.error
            )

        stub_result = _phase_dispatch(
            ctx, "3.3", matrix_result.output, StubManifest
        )
        if not stub_result.success:
            return PhaseResult(
                "architect", False, error=stub_result.error
            )
...
def _run_test(ctx: PhaseContext) -> PhaseResult:
    ...
    try:
        ...
        swarm_results = _dispatch_swarm(
            ctx, "4.2", swarm_inputs, FileOutput
        )
...
def _run_implement(ctx: PhaseContext) -> PhaseResult:
    ...
    try:
        ...
        swarm_results = _dispatch_swarm(
            ctx, "5.2", swarm_inputs, FileOutput
        )
        failures = [r for r in swarm_results if not r.success]
        if failures:
            errors = "; ".join(r.error or "unknown" for r in failures)
        ...
        integrate = _phase_dispatch(
            ctx, "5.4", issue_ctx, FileOutput
        )
        if not integrate.success:
            return PhaseResult(
                "implement", False, error=integrate.error
            )

        add_label(
            ctx.issue_number, "phase:implement", repo=ctx.repo
        )
